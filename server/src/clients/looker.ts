...
type LookerRow = Record<string, string | number | null | undefined>;
...
function parseLookerRows(raw: unknown): LookerRow[] {
  if (Array.isArray(raw)) {
    return raw as LookerRow[];
  }
  ...
  if (typeof raw === "string") {
    ...
    for (const line of raw.split("\n")) {
      ...
      try {
        rows.push(JSON.parse(trimmed) as LookerRow);
      } catch {
...
export async function runLookerQuery(args: LookerQueryArgs): Promise<LookerRow[]> {
...
export function readNumber(row: LookerRow, field: string): number {
  const value = row[field];
  return typeof value === "number" ? value : Number(value ?? 0);
}

export const LOOKER_GMV_FIELD =
  "fact_order_delivery.food_provider_price_before_discounts_eur_local";

export const LOOKER_ORDER_COUNT_FIELD =
  "fact_order_delivery.food_orders_created_local";

export const LOOKER_AVG_ORDER_FIELD =
  "fact_order_delivery.average_food_provider_price_before_discounts_eur_local";

export const LOOKER_ACTIVE_VENDORS_FIELD =
  "fact_order_delivery.delivery_vendors_with_orders_finished_local";

export const LOOKER_DATE_FIELD =
