import { LOOKER_ACTIVE_CITIES_EXPLORE, LOOKER_ACTIVE_CITIES_FIELD, LOOKER_AVG_ORDER_FIELD, LOOKER_CITY_FIELD, LOOKER_COUNTRY_FIELD, LOOKER_DATE_FIELD, LOOKER_ACTIVE_VENDORS_FIELD, LOOKER_GMV_FIELD, LOOKER_ORDER_COUNT_FIELD, readNumber, runLookerQuery, } from "../clients/looker.js";
import { countLeads, fetchRecentAccounts, fetchRecentLeads, } from "../clients/salesforce.js";
...
function cityStatus(growthPct) {
    if (growthPct >= 8)
        return { status: "Optimal", color: "primary" };
    if (growthPct >= 0)
        return { status: "Stable", color: "primary" };
    return { status: "Review", color: "error" };
}
function leadIcon(status) {
    if (status === "Converted") {
        return {
            icon: "verified",
...
async function queryScalar(field, filter) {
...
async function buildLookerSections(sources) {
    const [revenue30d, revenuePrev30d, activeVendors, activeVendorsPrev, activeCitiesRows, dailyRows, cityCurrentRows, cityPrevRows,] = await Promise.all([
    ...
    const avgDaily = dailyValues.reduce((sum, value) => sum + value, 0) /
        Math.max(dailyValues.length, 1);
    const weeklyRevenue = dailyRows.map((row) => {
    ...
    const cities = cityCurrentRows.slice(0, 8).map((row) => {
...
async function applySalesforceMetrics(metrics, sources) {
    const [leadsMtd, leadsPrevMonth, recentLeads, recentAccounts] = await Promise.all([
...
function placeholderModel(sources, error) {
    const message = error ??
